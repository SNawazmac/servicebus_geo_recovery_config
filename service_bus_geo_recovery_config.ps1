...
[Parameter(Mandatory=$true)][string]$primary_subscription_Id                                               
)

Set-AzContext -Subscription $primary_subscription_Id

New-AzServiceBusGeoDRConfiguration -Name $servicebus_aliasname -NamespaceName $primary_servicebus_namespace_name -ResourceGroupName $primary_servicebus_resource_group_name -PartnerNamespace $secondary_servicebus_namespaceId
